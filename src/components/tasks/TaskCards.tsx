        ...
        return (
            <div className="border rounded-lg p-4 mb-4 shadow-sm bg-white">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setIsEditing(false)} className="border border-gray-300 px-3 py-1 rounded hover:bg-gray-100">Cancel</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={`border rounded-lg p-4 mb-4 shadow-sm ${statusColors[task.status]}`}>
            <div className="flex justify-between items-start">
                ...
                <span className="text-sm font-medium px-2 py-1 rounded-full bg-gray-200">{task.status}</span>
            </div>
            ...
            <div className="mt-4 flex gap-2">
                {task.status !== TaskStatus.COMPLETED && (
                    <>
                        <button onClick={() => setIsEditing(true)} className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Edit</button>
                        <div className="flex items-center">
                            <input type='number' value={actualHours} 
                                onChange={(e) => setActualHours(parseInt(e.target.value))} 
                                className="border rounded w-16 px-2 py-1 mr-2" placeholder='Hours'
                            />
