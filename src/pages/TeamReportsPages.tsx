  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          <label htmlFor="team-select" className="block mb-2">Team:</label>
          <select
            id="team-select"
            value={selectedTeam}
        ...
        <div>
          <label htmlFor="sprint-select" className="block mb-2">Sprint:</label>
          <select
            id="sprint-select"
          ...
          <div className="bg-white p-4 rounded-lg shadow mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                ...
                <p className="text-3xl font-bold">{totalCompletedTasks} / {tasks.length}</p>
                <p className="text-sm text-gray-500">
                  {tasks.length > 0 ? `${(totalCompletedTasks / tasks.length * 100).toFixed(1)}%` : '0%'}
                </p>
              </div>
                    ...
                    {Object.values(developerStats).map((dev, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="border p-2">
                          {dev.tasksCount > 0 ? `${(dev.completedTasks / dev.tasksCount * 100).toFixed(1)}%` : '0%'}
                        </td>
